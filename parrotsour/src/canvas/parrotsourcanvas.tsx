 ...
 */
 export default abstract class ParrotSourCanvas extends React.PureComponent<PictureCanvasProps, PictureCanvasState> {
    ...
    constructor(props: PictureCanvasProps){
        ...
        this.state = {
            ...
            reDraw: ():PictureAnswer=>{ throw("Should not use parent reDraw") },
            answer: {pic:"", groups:[]}
        }
            ...
            if (ctx){
                if (animate){
                    if (animateCanvas){
                        ...
                        this.animationHandler.animate(ctx, this.props, this.state, answer.groups, animateCanvas, resetCallback);
                    }
                } else {
                    const callback =  () => drawFullInfo(ctx, this.state, braaFirst, showMeasurements, answer.groups)
    ...
    draw = async (_ctx: CanvasRenderingContext2D|null|undefined):Promise<void> => {
        return undefined
    }
