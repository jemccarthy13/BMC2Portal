...
import { DrawCanvasProps } from 'canvas/canvastypes'
 ...
 */
function DrawingCanvas(props: DrawCanvasProps):ReactElement {
    ...
    const mouseCanvasRef: React.RefObject<HTMLCanvasElement> = useRef<HTMLCanvasElement>(null)
    const mouseCvCtx: React.MutableRefObject<CanvasRenderingContext2D|null> = useRef(null)
    ...
    const { draw, orientation, braaFirst, bullseye, picType, 
    ...
    }, [draw, orientation, braaFirst, picType, showMeasurements, newPic, isHardMode, dataStyle])
     ...
     */
    const getMousePos = (canvas: HTMLCanvasElement|null, evt: CanvasMouseEvent): Point=> {
    ...
    const onMouseLeave = ()=>{
        if(mouseCvCtx.current)
            mouseCvCtx.current.clearRect(0,0, mouseCvCtx.current.canvas.width, mouseCvCtx.current.canvas.height)
    }
    ...
    <div style={{display:"block",textAlign:"left"}}>
        <div style={{display:"grid", position:"relative", height:"600px"}}>
            <canvas id="pscanvas" {...moveProps} style={{...style, gridColumn:"2", gridRow:"1", left:"0px"}} ref={canvasRef} />
            <canvas id="mousecanvas" {...moveProps} style={{...style, gridColumn:"2", gridRow:"1", position:"absolute", left:"0px",backgroundColor:"transparent"}} ref={mouseCanvasRef} />
        </div>
    </div>
    )
}
