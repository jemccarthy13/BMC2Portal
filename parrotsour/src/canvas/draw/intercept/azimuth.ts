...
import { drawAltitudes, drawMeasurement, getStartPos } from "canvas/draw/drawutils"
import { formatGroup, getGroupOpenClose } from "canvas/draw/formatutils"
import { isAnchorNorth, isEchelon, picTrackDir } from "canvas/draw/intercept/picturehelpers"
 ...
 */
 export const drawAzimuth:PictureDrawFunction = (
    ...
    let ng:AircraftGroup
    if (start){
      ng =  GroupFactory.randomGroupAtLoc(ctx, props, state, start)
    } else 
      ng = GroupFactory.randomGroup(ctx, props, state, {
    ...
    drawMeasurement(ctx, nPos.x, nPos.y + 2, m2.x, m2.y + 2, width, props.showMeasurements);

    drawAltitudes(ctx, nPos, ng.getAltitudes(), offsetX, offsetY);
    drawAltitudes(ctx, sg.getCenterOfMass(), sg.getAltitudes(), offsetX2, offsetY2);

    const ngBraaseye = new Braaseye(ng.getCenterOfMass(), state.bluePos.getCenterOfMass(), state.bullseye)
    const sgBraaseye = new Braaseye(sg.getCenterOfMass(), state.bluePos.getCenterOfMass(), state.bullseye)

    ngBraaseye.draw(ctx, props.showMeasurements, props.braaFirst, offsetX, offsetY)
    sgBraaseye.draw(ctx, props.showMeasurements, props.braaFirst, offsetX2, offsetY2)
    ...
    if (!anchorN){
        if (isNS){
          answer += formatGroup("EAST", sgBraaseye, sgAlts, sg.getStrength(), true, sg.getTrackDir());
          answer += " " + formatGroup("WEST", ngBraaseye, ngAlts, ng.getStrength(), includeBull, ng.getTrackDir());
        } else {
          answer += formatGroup("SOUTH", sgBraaseye, sgAlts, sg.getStrength(), true, sg.getTrackDir());
          answer += " " + formatGroup("NORTH", ngBraaseye, ngAlts, ng.getStrength(), includeBull, ng.getTrackDir());
        }
    } else {
        if (isNS){
          answer += formatGroup("WEST", ngBraaseye, ngAlts, ng.getStrength(), true, ng.getTrackDir());
          answer += " " + formatGroup("EAST", sgBraaseye, sgAlts, sg.getStrength(), includeBull, sg.getTrackDir());
        } else {
          answer += formatGroup("NORTH", ngBraaseye, ngAlts, ng.getStrength(), true, ng.getTrackDir());
          answer += " " + formatGroup("SOUTH", sgBraaseye, sgAlts, sg.getStrength(), includeBull, sg.getTrackDir());
        }
    }
