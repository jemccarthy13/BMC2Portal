...
function checkForCoAltitude(groups1:AircraftGroup[], groups2:AircraftGroup[]){
  const result = groups1.filter((grp:AircraftGroup) => groups2.some(grp2 => 
    {
      grp.getAltitude() === grp2.getAltitude() && grp.getCenterOfMass().getBR(grp2.getCenterOfMass()).range <= 20
    }))
  if (result.length !== 0 ) {
    ...
    for (let x = 0; x < groups.length; x++) {
      ...
      if (!groups[x].isCapping() && !groups[x].hasRouting()){
        ...
        if( groups[x].hasRouting()){
          newHeading = groups[x].getCenterOfMass().getBR(groups[x].getNextRoutingPoint()).bearingNum;
        }
        ...
        drawGroupCap(ctx, props.orientation.orient, groups[x].getStrength(), sPos.x,sPos.y, "blue")
      }  
      const sPos = groups[x].getCenterOfMass()
