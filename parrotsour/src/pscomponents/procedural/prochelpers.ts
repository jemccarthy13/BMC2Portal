...
export function aiProcess(
  ...
  const move3d = nl.match(
    "[<cs>#Noun] [#Verb?] * [<x>#Cardinal] [<y>#Cardinal] app? [#Verb?] [#Preposition?] #Unit [<fl>#Cardinal]"
  )
  const desert3d = nl.match(
    "[<cs>#Noun] [#Verb?] * [<x>#Cardinal] [<y>#Cardinal] * app? * [#Verb?] [#Preposition?] #Unit [<fl>#Cardinal]"
  )
  const desert3d2 = nl.match(
    "[<cs>#Noun] [#Verb?] * [<x>#Cardinal] [<y>#Cardinal] * [#Verb?] [#Preposition?] #Unit [<fl>#Cardinal] * app?"
  )
  ...
  if (isMove) {
    ...
    sendResponse(
      callsign,
      cpy + cmd.groups().act.verbs().toGerund().text() + " " + newfl,
      msg.voice
    )
    asset.updateIntent({
