...
export class Point {
    ...
    getBR(toPoint:Point): BRAA {
        ...
        const rng = Math.floor(Math.sqrt(deltaX * deltaX + deltaY * deltaY) / PIXELS_TO_NM);
  
        // convert cartesian direction to heading
        let brg = Math.round(270 + toDegrees(Math.atan2(this.y - toPoint.y, this.x - toPoint.x)));
        if (brg > 360) {
            brg = brg - 360;
        }
     ...
     */
    public straightDistNM(toPoint:Point, orientation: FightAxis): number {
        return Math.floor(Math.abs((orientation === FightAxis.NS ? this.y - toPoint.y : this.x - toPoint.x)) / PIXELS_TO_NM)
    }
}